...
float tsp::read_file(const char * filename)
{
    float added = 0;
    float id_read = 0;
    float x_read = 0;
    ...
    while ( ss >> line && line != "EOF" ) //go till end of file
    {
...
void tsp::brute_force(deque <city*> & best_path, float & min_distance, float cities_left)
{
...
float tsp::nearest_neighbor_basic( int start_index)
{
...
float tsp::two_change()
{
    deque <city*> new_path;
    float min_distance = get_solution_distance();
...
float tsp::swap_two(int i, int k)
{
    deque <city*> temp;
    int count = 0;
...
float tsp::get_solution_distance()
{
    float total_dist = 0;
...
{
    float distance = get_solution_distance();
